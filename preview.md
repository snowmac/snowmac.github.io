...
**Young generation.** New allocations land here first, in a small, fast space. V8's young-generation collector (nicknamed Scavenger) has traditionally used semi-space copying: the young generation is split into two halves, and everything still reachable gets copied from the active half to the other on each collection. Copying is fast because you're only touching live objects, which is why short-lived objects are nearly free in JavaScript. Recent V8 releases have increasingly adopted a project called Minor Mark-Sweep (Minor MS) for parts of this work instead, collecting the young generation with mark-sweep rather than copying, which reduces the memory overhead of keeping two full semi-spaces around while preserving the same generational behavior and staying parallelized across worker threads. The mental model (short-lived objects are cheap to collect) holds either way; the exact mechanism underneath it is still actively evolving.
...
- **In the browser**, the Performance Monitor tab (separate from the Memory panel) gives a live, scrubbable graph of JS heap size, DOM node count, and listener count over time. Leave it running while you use the app normally and watch the trend, not any single point (the sawtooth pattern from earlier is exactly what a healthy graph looks like here).
...
- **RSS vs. `heapUsed`** matters here too: RSS (resident set size) includes memory outside V8's managed heap entirely. If RSS grows but `heapUsed` stays flat, check these three places before assuming a native module:
  - `Buffer`/`ArrayBuffer` allocations, which live in the `external` field of `process.memoryUsage()`, not `heapUsed`
  - An unclosed resource: a database connection, an open file handle, an un-destroyed stream
  - Allocator fragmentation. Under high concurrency, Linux's default glibc allocator is genuinely bad about handing freed memory back to the OS, so V8 can have correctly freed something at the JS level while RSS still looks high. Swapping in `jemalloc` or `tcmalloc` (via `LD_PRELOAD`) is a standard production fix for exactly this "phantom leak," not a JavaScript-level bug at all
...
5. Use the "Comparison" view between snapshots two and three. Anything that grew and didn't shrink after a forced GC is either a real leak or something that legitimately needs to stay alive, and DevTools shows you the retaining path directly. That's the diagram from the beginning of this post, rendered for your actual application instead of a toy example.
...
- **`v8.writeHeapSnapshot()`** is built directly into Node core: zero dependencies, no native compilation, safe to wire up behind a signal in a running service:
  ...
  Trigger it against a live production process (`kill -USR2 <pid>`) and load the resulting `.heapsnapshot` file in Chrome DevTools exactly like a browser snapshot. This is the tool I'd reach for first now, precisely because it ships with Node itself. One important caveat: it's synchronous and fully blocks the event loop while it serializes the heap to disk. On a service with a multi-gigabyte heap under real traffic, that can stall the process for several seconds, long enough for a load balancer or Kubernetes liveness probe to decide the instance is dead and restart it. Pull the instance out of rotation before signaling it in production, or trigger it against a dedicated replica instead.
- **`node --heap-prof`** and **`node --cpu-prof`** are also built directly into Node and write a `.heapprofile` or `.cpuprofile` file on exit (or on signal), both natively importable into Chrome DevTools, with nothing to install.
...
- **`AbortController`**: the modern, clean way to tie a listener's lifetime to something else's, and it's not just a browser API. In the browser, pass the same `signal` to every `addEventListener` call for a component's lifetime and call `.abort()` once instead of pairing up individual `removeEventListener` calls. In Node, `EventEmitter.on()` accepts the same `{ signal }` option natively:
  ...
  app.get('/status', (req, res) => {
    ...
    // fires whether the client disconnects, the response finishes normally,
    // or the stream errors, unlike relying on res.on('close') alone
    finished(res, () => ac.abort());

    emitter.on('update', (data) => res.write(data), { signal: ac.signal });
  });
  ```

  This is the direct fix for the per-request `EventEmitter` leak from earlier: one `.abort()` call instead of remembering to `.removeListener()` on every exit path. Reaching for `node:stream`'s `finished()` here instead of just `res.on('close')` matters: `close` alone can miss or delay firing depending on Node version, keep-alive behavior, and whether an error happened mid-stream, while `finished()` is the API Node core provides specifically to unify all three completion paths.
...
Garbage collectors don't understand intent. They don't know you're finished with an object, only whether something can still reach it. Every leak in this post, from a detached DOM node to a forgotten EventEmitter listener, is the same story: a reference that outlived its purpose.

What changes between a frontend app and a Node service is the blast radius. A browser leak degrades one person's tab until they close it. A Node leak degrades your entire service until it crashes, because there's no reload button on a server that's supposed to stay up for weeks. Learn to follow the retaining path instead of staring at a single heap-size number, and treat every module-level cache, every event listener, and every timer as something that needs an explicit exit plan, not just a start.
